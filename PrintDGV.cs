...
namespace PrintDataGrid
{
    class PrintDGV
    {
        ...
        public static void Print_DataGridView(DataGridView dgv1, string Title, bool Landscape)
        {
            PrintTitle = Title;
        ...
        public static void Print_DataGridView(DataGridView dgv1)
        {
            try
            {
                ...
                // Zobrazi nastaveni tisku
                PrintOptions dlg = new PrintOptions(AvailableColumns, dgv);
        ...
        public static void OnlyPreview(DataGridView dgv1, string Title, bool AllRows, bool Fit, List<string> Cols)
        {
                    ...
                    System.Drawing.Printing.PrintEventArgs e) 
        {
            try
	        {
                ...
                StrFormatComboBox.LineAlignment = StringAlignment.Near;
                    ...
                    else
                    {
                        ...
                        foreach (DataGridViewCell Cel in GridRow.Cells)
                        {
                            ...
                            if (((Type)ColumnTypes[i]).Name == "DataGridViewTextBoxColumn" ||
                                ...
                                ((Type)ColumnTypes[i]).Name == "MaskedTextBoxColumn")
                            {
                                e.Graphics.DrawString(Cel.Value.ToString(), Cel.InheritedStyle.Font, 
                                        new SolidBrush(Cel.InheritedStyle.ForeColor),
                                        new RectangleF((int)ColumnLefts[i], (float)tmpTop + 2,
                                        (int)ColumnWidths[i], (float)CellHeight - 2), StrFormat);
                            }
                            ...
                            {   
                                ...
                                //po zmene comboboxu na jinou value se tam vrazi hlavicka
                                
                                if (last_kat == 0) //zaciname
                                {
                                    last_kat2 = last_kat = int.Parse(Cel.Value.ToString());
                                }
                                else //uz to neni prvni radek...
                                {
                                    last_kat = (int)Cel.Value;
                                }


                                e.Graphics.DrawString(Cel.FormattedValue.ToString(), Cel.InheritedStyle.Font, 
                                        new SolidBrush(Cel.InheritedStyle.ForeColor), 
                                        new RectangleF((int)ColumnLefts[i] + 1, tmpTop + 2, (int)ColumnWidths[i]
                                        - 16, CellHeight - 2), StrFormatComboBox);
                            }
                        ...
                        if (last_kat != last_kat2)
                        {
                            last_kat2 = last_kat;
                            e.Graphics.DrawLine(new Pen(Brushes.Black, 3), (int)ColumnLefts[0],
                                        tmpTop, (int)ColumnLefts[0] + e.MarginBounds.Width - 3, tmpTop);
                        }
                        tmpTop += CellHeight;
                    }
